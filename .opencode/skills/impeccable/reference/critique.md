...
2. **Confirm the target slugs cleanly**:
   ```bash
   .opencode/skills/impeccable/scripts/impeccable critique-storage slug "<resolved-path-or-url>"
   ```
...
```bash
.opencode/skills/impeccable/scripts/impeccable detect --json [target]
```
...
4. If mutation is available, start `.opencode/skills/impeccable/scripts/impeccable live-server --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     .opencode/skills/impeccable/scripts/impeccable critique-storage write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ...
   ```bash
   .opencode/skills/impeccable/scripts/impeccable critique-storage trend "<resolved target>" 5
   ```
